...
def extract_post_and_comments_cleaned(json_file_path):
    ...
    # Store in DataFrame
    post_df['embeddings'] = post_embeddings_list  
    post_df['emotion_probs'] = post_emotion_probs_list 
    comments_df['embeddings'] = None  
    for idx, embedding in enumerate(embeddings_list):
        comments_df.at[idx, 'embeddings'] = embedding

    comments_df['emotion_probs'] = emotion_probs_list  
...
post_df.to_csv('../comments_w_embeddings/post_df_with_embeddings.csv', index=False)
comments_df['emotion_probs'] = comments_df['emotion_probs'].apply(lambda probs: {k: float(v) for k, v in probs.items()})

comments_df.to_csv('../comments_w_embeddings/comments_df_with_embeddings.csv', index=False)
