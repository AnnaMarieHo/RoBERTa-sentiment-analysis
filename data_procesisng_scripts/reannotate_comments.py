...
def process_batch(start_idx, batch_size, filename):
    ...
    for i in range(start_idx, len(goemotions), batch_size):
        ...
        save_to_csv(batch_results, filename=filename)
        batch_results.clear()  

        INSTRUCTION_TEMPLATE = get_user_feedback(INSTRUCTION_TEMPLATE)  
...
def save_to_csv(results, filename, mode='a'):
    ...
    #     df = df[~df["text"].isin(existing_df["text"])]  
...
if __name__ == "__main__":
    process_batch(start_idx=0, batch_size=2, filename="reannotated_goemotions.csv") #filename refers to the output file
